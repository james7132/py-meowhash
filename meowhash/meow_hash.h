...
#if UPDATE MEOW_HASH_SPECIALIZED##Update
#if FLUSH MEOW_HASH_SPECIALIZED##Flush

static void UPDATE(meow_state *Block, meow_64 Len, void *SourceInit) {
  // NOTE(casey): Handle as many full 256-byte blocks as possible
  // TODO(james7132): Handle residual partial from previous update call
  ...
  if (Len) {
    State->PartialLen = Len;
    // TODO(casey): Can this just be zeroes?
    State->Partial[] = {Block->IV, Block->IV, Block->IV, Block->IV};
    meow_u8 *Dest = (meow_u8 *)State->Partial;
    while (Len--) {
      *Dest++ = *Source++;
    }
  }
...
static meow_lane FLUSH(meow_state State) {
  // NOTE(casey): If residual data remains, hash one final 256-byte block padded
  // with the initialization vector
  if (state.PartialLen) {
    meow_u8 *Partial = (meow_u8 *)state.Partial;
  ...
  // NOTE(casey): Combine the 16 streams into a single hash to spread the bits
  // out evenly
  meow_lane R0 = State->IV;
  ...
  AESRotate(R0, state.S89AB);
  AESRotate(R0, state.SCDEF);
  // NOTE(casey): Repeat AES enough times to ensure diffusion to all bits in
  // each 128-bit lane
  AESMerge(R0, state.IV);
  AESMerge(R0, state.IV);
  AESMerge(R0, states.IV);
  AESMerge(R0, state.IV);
  AESMerge(R0, state.IV);
...
static meow_lane MEOW_HASH_SPECIALIZED(meow_u64 Seed, meow_u64 Len,
                                       void *SourceInit) {
  meow_state state;
  MewoStateInit(&state, Seed);
  UPDATE(&state, Len, SourceInit);
  return FLUSH(state);
}
...
static void MeowStateInit(meow_state *Block, meow_64 Seed) {
  // NOTE(casey): The initialization vector follows falkhash's lead and uses the
  // seed twice, but the second time the length plus one is added to
  // differentiate.  This seemed sensible, but I haven't thought too hard about
  // this, there may be better things to use as an IV.
  meow_lane IV;
...
typedef meow_lane meow_hash_implementation(meow_u64 Seed, meow_u64 Len,
                                           void *SourceInit);

#define MEOW_HASH_TYPES
#endif

static void MeowAESRotate128x4(meow_lane &A, meow_lane &B) {
...
#if defined(MEOW_HASH_256)
static void MeowAESRotate256x2(meow_lane &A, meow_lane &B) {
...
#if defined(MEOW_HASH_512)
static void MeowAESRotate512(meow_lane &A, meow_lane &B) {
...
#define AESLoad(S, From) S.Q0 = _mm512_aesdec_epi128(S.Q0, *(__m512i *)(From))
#define AESMerge(A, B) A.Q0 = _mm512_aesdec_epi128(A.Q0, B.Q0);
